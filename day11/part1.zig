...
fn literalArrayList(allocator: Allocator, items: anytype) !ArrayList(i32) {
    const items_fields = @typeInfo(@TypeOf(items)).Struct.fields;

    var ary = try ArrayList(i32).initCapacity(allocator, items_fields.len);
