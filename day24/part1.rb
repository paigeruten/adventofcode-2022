...
loop do
  node, idx = queue.each.with_index.min_by { |node, idx| dist[node] || Float::INFINITY }
  queue.delete_at(idx)
  visited.add(node)
  ...
  find_neighbours(node, blizzard_maps, width, height).each do |neighbour|
    ...
    dist[neighbour] = [dist[neighbour] || Float::INFINITY, dist[node] + 1].min
    queue << neighbour unless queue.include? neighbour

    if neighbour[0] == width && neighbour[1] == height + 1
      p dist[neighbour]
      exit
    end
  end
