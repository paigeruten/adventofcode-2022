...
pub fn main() !void {
    ...
    while (try reader.readUntilDelimiterOrEof(&line_buf, '\n')) |line| {
        ...
        _ = it.next().?;
        const from_stack = try parseInt(usize, it.next().?, 10) - 1;
        _ = it.next().?;
        const to_stack = try parseInt(usize, it.next().?, 10) - 1;

        var i: usize = 0;
        while (i < count) : (i += 1) {
            const crate = stacks.items[from_stack].pop();
            try stacks.items[to_stack].append(crate);
