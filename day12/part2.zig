...
const Board = struct {
    ...
    fn findShortestPathLength(self: *const Board, allocator: std.mem.Allocator) !usize {
        ...
        for (self.end_idxs) |end_idx| {
            if (dist.items[end_idx] < min_length) {
                min_length = dist.items[end_idx];
            }
        }
    ...
    fn elevation(self: *const Board, pos: Position) usize {
        return 27 - switch(self.grid[pos.toIndex(self.width)]) {
            'S' => 1,
            'E' => 26,
            else => |char| char - 'a' + 1,
        };
...
pub fn main() !void {
    ...
    var end_idxs = std.ArrayList(usize).init(allocator);
    defer end_idxs.deinit();
    ...
    while (reader.readByte() catch null) |char| {
        if (char == '\n') {
            ...
            height += 1;
            continue;
        } else if (char == 'S' or char == 'a') {
            try end_idxs.append(i);
        } else if (char == 'E') {
            start_idx = i;
        }
